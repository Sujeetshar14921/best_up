...
export default function RecommendPage() {
  ...
  const priorities = [
    { value: 'Gaming', label: 'Gaming', icon: Zap, color: 'from-red-500 to-orange-500' },
    { value: 'Camera', label: 'Camera', icon: Target, color: 'from-pink-500 to-purple-500' },
    { value: 'Vlogging', label: 'Vlogging', icon: Target, color: 'from-blue-500 to-cyan-500' },
    { value: 'Battery', label: 'Battery', icon: Zap, color: 'from-green-500 to-emerald-500' },
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full mb-4">
            ...
            <span className="text-sm font-semibold text-blue-600">AI-Powered Recommendations</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            Find Your <span className="text-yellow-400">Best</span><span className="text-blue-600">Up</span> Phone
          </h1>
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Answer a few questions about your needs and budget, and get personalized phone recommendations tailored just for you.
          </p>
        </div>
        ...
        {!submitted || topThreePhones.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 md:p-12 mb-12">
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <div>
                ...
                <div className="space-y-6">
                  <input
                    ...
                    className="w-full h-3 bg-gradient-to-r from-blue-400 to-blue-600 rounded-lg appearance-none cursor-pointer"
                  />
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <p className="text-gray-600 text-sm mb-1">Selected Budget</p>
                      <span className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-yellow-400 to-blue-600 bg-clip-text text-transparent">
                  ...
                  {priorities.map(({ value, label, icon: Icon }) => (
                    <button
                      ...
                      className={`p-4 rounded-xl font-semibold transition-all flex flex-col items-center gap-2 border-2 ${
                        priority === value
                          ? 'bg-gradient-to-br from-blue-600 to-blue-700 text-white shadow-lg scale-105 border-blue-700'
              ...
              <button
                type="submit"
                className="w-full py-4 bg-gradient-to-r from-yellow-400 to-blue-600 text-white rounded-xl font-bold text-lg hover:shadow-xl transition-all hover:scale-105 flex items-center justify-center gap-2"
              >
                Get Recommendations
                <ArrowRight size={24} />
              </button>
          ...
          <button
            ...
            className="mb-8 px-6 py-2 bg-white rounded-lg border border-gray-200 font-semibold hover:bg-gray-50 transition-all"
          >
            ← Change Preferences
          </button>
        )}
                  ...
                  {winner && (
                    <div className="bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50 border-2 border-yellow-300 rounded-2xl p-8 text-center mt-8 shadow-lg">
                      ...
                      <div className="mb-6">
                        ...
                        <div className="bg-white rounded-xl p-8 inline-block shadow-md">
                          <h3 className="text-2xl font-bold text-gray-900 mb-2">{winner.name}</h3>
                          <p className="text-gray-600 mb-3">{winner.brand}</p>
                          <p className="text-4xl font-bold bg-gradient-to-r from-yellow-400 to-blue-600 bg-clip-text text-transparent mb-6">₹{(winner.basePrice / 1000).toFixed(0)}K</p>
                          
                          {/* Performance Metrics */}
                          <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-gray-200">
                            {priority === 'Value' && winner.scores?.valueForMoney && (
                              <div className="bg-green-50 rounded-lg p-3"><p className="text-green-600 font-bold text-lg">{winner.scores.valueForMoney.toFixed(1)}/10</p><p className="text-green-700 text-xs font-semibold">Value Score</p></div>
                            )}
                            {priority === 'Gaming' && winner.scores?.gaming && (
                              <div className="bg-red-50 rounded-lg p-3"><p className="text-red-600 font-bold text-lg">{winner.scores.gaming.toFixed(1)}/10</p><p className="text-red-700 text-xs font-semibold">Gaming Score</p></div>
                            )}
                            {(priority === 'Camera' || priority === 'Vlogging') && winner.scores?.camera && (
                              <div className="bg-pink-50 rounded-lg p-3"><p className="text-pink-600 font-bold text-lg">{winner.scores.camera.toFixed(1)}/10</p><p className="text-pink-700 text-xs font-semibold">Camera Score</p></div>
                            )}
                            {priority === 'Battery' && winner.scores?.battery && (
                              <div className="bg-cyan-50 rounded-lg p-3"><p className="text-cyan-600 font-bold text-lg">{winner.scores.battery.toFixed(1)}/10</p><p className="text-cyan-700 text-xs font-semibold">Battery Score</p></div>
                            )}
                            {winner.baseSpecs && (
                              <div className="bg-blue-50 rounded-lg p-3"><p className="text-blue-600 font-bold text-lg">{winner.baseSpecs?.ram || 'N/A'}</p><p className="text-blue-700 text-xs font-semibold">RAM</p></div>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      {/* CTA Button */}
                      <button onClick={() => window.location.href = `/details/${winner._id}`} className="mt-6 px-8 py-3 bg-gradient-to-r from-yellow-400 to-blue-600 text-white font-bold rounded-lg hover:shadow-lg transition-all">
                        View Full Details
                      </button>
                    </div>
                  )}
            ...
            {[
              {
                icon: Flame,
                title: 'Gaming Score',
                description: 'Performance, refresh rate, and cooling system analysis',
                color: 'text-red-600',
                bgColor: 'bg-red-50'
              },
              {
                icon: Target,
                title: 'Camera Score',
                description: 'Sensor quality, OIS, zoom range, and video capabilities',
                color: 'text-pink-600',
                bgColor: 'bg-pink-50'
              },
              {
                ...
                description: 'Performance-to-price ratio for the best value phones',
