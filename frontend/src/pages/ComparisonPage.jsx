    ...
    <div className="min-h-screen bg-light py-12">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {selectedPhones.map((phone) => (
              <div key={phone._id} className="bg-gradient-to-br from-yellow-50/50 to-orange-50/50 rounded-lg p-4 border-2 border-yellow-300 relative">
                {overallWinner?._id === phone._id && (
                  <div className="absolute -top-3 -right-3 bg-gradient-to-r from-yellow-400 to-orange-600 text-white rounded-full p-2 shadow-lg">
                    <Trophy size={20} />
                  </div>
                )}
                <div className="flex justify-between items-start mb-2">
                  ...
                  <button
                    ...
                    className="p-1 hover:bg-orange-100 rounded-lg transition-all"
                  >
                    <X size={18} className="text-orange-600" />
                  </button>
                </div>
                <p className="text-sm text-gray-600 mb-2">{phone.brand}</p>
                <p className="text-lg font-bold bg-gradient-to-r from-yellow-600 to-orange-600 bg-clip-text text-transparent">₹{(phone.basePrice / 1000).toFixed(0)}K</p>
              </div>
            ))}
            ...
            {selectedPhones.length < 3 && (
              <button
                ...
                className="bg-white border-2 border-dashed border-yellow-400 rounded-lg p-4 hover:bg-yellow-50/50 transition-all flex flex-col items-center justify-center min-h-24"
              >
                <Search size={24} className="text-yellow-600 mb-2" />
                <span className="font-semibold text-yellow-700">Add Phone</span>
              </button>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                {filteredPhones.map((phone) => (
                  <button
                    ...
                    className="p-4 border-2 border-gray-200 rounded-lg hover:border-yellow-400 hover:bg-yellow-50/50 transition-all text-left"
                  >
                    <h4 className="font-bold text-gray-900">{phone.name}</h4>
                    <p className="text-sm text-gray-600">{phone.brand}</p>
                    <p className="text-lg font-bold bg-gradient-to-r from-yellow-600 to-orange-600 bg-clip-text text-transparent mt-2">₹{(phone.basePrice / 1000).toFixed(0)}K</p>
                  </button>
                ))}
              </div>
            </LoadingError>
        ...
        {selectedPhones.length >= 2 && overallWinner && (
          <div className="mb-8 bg-gradient-to-r from-yellow-50 via-orange-50 to-yellow-50 rounded-2xl shadow-lg border-2 border-yellow-300 overflow-hidden relative">
            {/* Animated Background */}
            ...
            <div className="relative p-8 z-10">
              ...
              <div className="flex items-center justify-center gap-3 mb-6">
                ...
                <h2 className="text-4xl font-black bg-gradient-to-r from-yellow-600 to-orange-600 bg-clip-text text-transparent">BEST CHOICE</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-xl shadow-xl p-6 border-2 border-yellow-400">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-4xl font-black bg-gradient-to-r from-yellow-600 to-orange-600 bg-clip-text text-transparent">₹{(overallWinner.basePrice / 1000).toFixed(0)}K</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                      ...
                      <span className="text-lg font-bold text-yellow-600">{overallWinner.scores?.gaming?.toFixed(1) || 'N/A'}/10</span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                      ...
                      <span className="text-lg font-bold text-orange-600">{overallWinner.scores?.camera?.toFixed(1) || 'N/A'}/10</span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                      <div className="flex items-center gap-2">
                      ...
                      <span className="text-lg font-bold text-yellow-600">{overallWinner.scores?.battery?.toFixed(1) || 'N/A'}/10</span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                      <div className="flex items-center gap-2">
                      ...
                      <span className="text-lg font-bold text-orange-600">{overallWinner.scores?.valueForMoney?.toFixed(1) || 'N/A'}/10</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {categoryWinners.camera?._id === overallWinner._id && (
                      <div className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                        <Camera size={20} className="text-orange-600 flex-shrink-0 mt-1" />
                        <div>
                    ...
                    {categoryWinners.battery?._id === overallWinner._id && (
                      <div className="flex items-start gap-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                        <Battery size={20} className="text-yellow-600 flex-shrink-0 mt-1" />
                        <div>
                    ...
                    {categoryWinners.value?._id === overallWinner._id && (
                      <div className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                        <DollarSign size={20} className="text-orange-600 flex-shrink-0 mt-1" />
                        <div>
        ...
        {selectedPhones.length >= 2 && (
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <table className="w-full">
              <thead className="bg-gradient-to-r from-yellow-50 to-orange-50 border-b-2 border-yellow-300">
                <tr>
        ...
        {selectedPhones.length === 0 && !showSearch && (
          <div className="text-center py-20 bg-white rounded-lg border-2 border-dashed border-yellow-300">
